...
def load_metrics_df() -> pd.DataFrame:
    try:
        ...
        # Read only the real metrics columns. Some sheets may have many extra
        # blank columns, which makes gspread get_all_records() fail because the
        # header row contains duplicate blank headers.
...
def render_product_name_safety_section(
    ...
    *,
    ...
    with st.expander("Rows over Product Name limit", expanded=True):
        st.dataframe(issues_df, width="stretch")

    with st.expander("Product shortening rules", expanded=True):
        ...
        optimized_lengths = optimized_df["Product Name"].apply(lambda value: len(str(value)) if pd.notna(value) else 0)
